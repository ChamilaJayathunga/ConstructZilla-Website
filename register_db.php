        ...
        if (empty($username)) {
            ...
            try {
                ...
                if ($row['username'] == $username) {
                    ...
                    $insert_stmt = $db->prepare("INSERT INTO users (username, email, password, role, Phone, Address) VALUES (:uname, :uemail, :upassword, :urole, :uphone,:uaddress)");
                    $insert_stmt->bindParam(":uname", $username);
                    $insert_stmt->bindParam(":uemail", $email);
                    $insert_stmt->bindParam(":upassword", $password);
                    $insert_stmt->bindParam(":urole", $role);
                    $insert_stmt->bindParam(":uphone", $phone);
                    $insert_stmt->bindParam(":uaddress", $address);

                    if ($insert_stmt->execute()) {
                        $_SESSION['success'] = "Register Successfully.<br>Login In To System";
